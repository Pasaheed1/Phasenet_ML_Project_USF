...
def read_args():
    ...
    parser.add_argument("--config_name", default="full", help="Experiment config name from experiment_configs.py")
    parser.add_argument("--disable_augmentation", action="store_true", help="Disable data augmentation")
    parser.add_argument("--temporal_loss_weight", default=0.1, type=float, help="Weight for temporal consistency loss")
...
def train_fn(args, data_reader, data_reader_valid=None):
    ...
    exp_config = get_config(args.config_name)
    
    config = ModelConfig(
       X_shape=data_reader.X_shape,
       Y_shape=data_reader.Y_shape,
       depths=exp_config['depths'],
       filters_root=exp_config['filters_root']       
    )
    ...
    with tf.compat.v1.Session(config=sess_config) as sess:
        ...
        if args.load_model and args.model_dir is not None:
